...
vi.mock("./Card", () => ({
    default: ({ title, children }) => createElement(
        "section",
        null,
...
describe("WeatherCard", () => {
    it("renders current weather details", () => {
        const weather = {
            ...
            iconUrl: "https://openweathermap.org/img/wn/10d2x.png",
        };

        render(createElement(WeatherCard, { weather }));

        expect(
            screen.getByRole("heading", { name: /current weather/i })).toBeInTheDocument();
    ...
    it("renders fallback error content when weather data is unabvalible", () => {
        render(
        ...
        expect(screen.getByText("Unable to load current weather right now.")).toBeInTheDocument();
